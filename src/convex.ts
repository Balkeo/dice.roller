...
export function geometryToConvexArgs(geom: THREE.BufferGeometry): ConvexArgs {
  ...
  if (indexArray.length % 3 !== 0)
    throw new Error("Geometry index/position not divisible by triangles.");
...
export function buildFaceGroups(geom: THREE.BufferGeometry): {
  ...
  const eps = 1e-3;
  const keyFor = (n: THREE.Vector3) =>
    `${Math.round(n.x / eps)},${Math.round(n.y / eps)},${Math.round(
      n.z / eps,
    )}`;
  ...
  for (let t = 0; t < triCount; t++) {
    ...
    let g = map.get(k);
    if (!g) {
      g = { normal: N.clone(), center: new THREE.Vector3(), triIndices: [] };
      map.set(k, g);
    }
    g.triIndices.push(t);
  }
