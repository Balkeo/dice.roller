...
(function(dice) {
    ...
    this.dice_box = function(container, dimentions) {
        ...
        this.world.addContactMaterial(new CANNON.ContactMaterial(
                    this.dice_body_material, this.dice_body_material, { friction: 0.01, restitution: 0.5 }));

        this.world.add(new CANNON.Body({ mass: 0, material: desk_body_material, shape: new CANNON.Plane() }));
        var barrier;

        barrier = new CANNON.Body({ mass: 0, material: barrier_body_material, shape: new CANNON.Plane() });
    ...
    this.dice_initialize = function(container) {
       const box = new this.dice_box(container, { w: container.clientWidth, h: container.clientHeight });
       ...
       box.camera = new THREE.PerspectiveCamera(45, container.clientWidth / container.clientHeight, 1, 10000);
       box.camera.position.set(0, 100, 100);
       box.scene.add(box.camera);

       const ambient = new THREE.AmbientLight(0xf0f0f0);
       box.scene.add(ambient);
       const directionalLight = new THREE.DirectionalLight(0xffffff, 0.5);
       directionalLight.position.set(0, 200, 100).normalize();
       box.scene.add(directionalLight);

       function animate() {
           requestAnimationFrame(animate);
           box.world.step(1.0 / 60.0);
